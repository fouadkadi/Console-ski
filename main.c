...
struct VSkieur //Champs du vecteur skieur.
{
    char Nom[25];
    char adresse[50];
    int psk; //Nombre de paires de skis louées.
...
PaireDeSkis *chargementski()
{
    ...
    for(t=100;t<320;t+=20)
    {
        ...
        while (i<5)
        {
            allouerski(&S);
            aff_valski(S,t);
            aff_adrski(P,S);// Liaison de S et P ie: de S et T(P étant un pointeur de meme valeur que T après la copie)
            P->loc=NULL;
            P=S; //Passer à l'element suivant
            i++;
        }
...
//-----------------------------------------------------Insertion----------------------------------------------------------
//*******************Procedure verifiant si la location peut être réalisée, renvoyant un booléen **************************
void testloc(PaireDeSkis* p,InfoSkis* q,int t, int d , int f,int *v,InfoSkis **actu,InfoSkis **suiv, PaireDeSkis **x)
{
  ...
  do
  {
   if((q)!=NULL)
    {
    if (f<(q->deb))// Test sur le debut ie : (Insertion avant la tête)
    {
        res=1;
        *actu=NULL;
        *suiv=q;
    ...
    else
    {
        while((q)!=NULL)
        {
          if((suivantloc(q))!=NULL)
          {
            ...
            {
              if((q->fin)<d)// Test sur la fin (Insertion après la queue)
              {
                  res=1;
                  *actu=q;
                  *suiv=NULL;
...
int location(PaireDeSkis* p,int t,int a, int b,char nom[25],vecteur vct)
{
    ...
    if (v==1)// la location peut etre réalisé
    {
...
InfoSkis *S;
        ...
        if (w!=NULL && w->loc==NULL) //La sous_liste de reservation n'a pas encore été crée,ie: 1er location de la taille en question
        {
            allouerloc(&w->loc);
        ...
        else
        {
            ...
            if (actu==NULL)// Inserer avant la tête.
            {
                allouerloc(&S);
                aff_valloc(S,a,b);
                aff_adrloc(S,suiv);
...
//*******************************************Résérvation(plusieurs locations) ********************************************
...
void reservation(PaireDeSkis *p,int nombredereservation,char client[25] ,char adr[50],vecteur *vgrand )
{
    ...
    for(i=0;i<nombredereservation;i++)
     {  label1:textbackground(15);
      ...
      if(location(skit(p,taille),taille,db,fn,client,*vgrand)==1)// Location réalisable
       {
       ...
       {
           if(skit(p,taille)==NULL)// Taille non répertoriée.
    ...
    vgrand->nbr --;} //Le client n'a pas réussie a faire au moin une resrvation.
}
//*******************************************************Annulation ************************************************************
    ...
    m=q;
        ...
        while(q!=NULL && j<vgrand->T[chercheskieur(client,*vgrand)].reservect->nbres)
        {
            ...
            while(z!=NULL && j<vgrand->T[chercheskieur(client,*vgrand)].reservect->nbres )
            {
                ...
                if (q->loc->ind==chercheskieur(client,*vgrand) && l==NULL)//Suppression au début
                {
...
{ int cpt,gpt ,i,y;InfoSkis *tetesousliste;
   ...
   //si la sous-liste est vide on dit que le ski est libre de 1 a  151
    ...
    cpt++;//ccompte le le numero du ski de taille damandée en court de traitment
      y=wherey()+1;
           for(i=0;i<16;i++){
        gotoxy(2*i+8,y);
        textbackground(15);
